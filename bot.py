...
async def start(message: types.Message):
    ...
    if token_db.get_balance(message.from_user.id) is None:
        ...
        await message.answer_photo(
            photo=photo_url, caption="Привет! 👋 Добро пожаловать в нашего бота, созданного для обучения в онлайн! Здесь вы сможете:\n\n"
        ...
        # Отправка сообщения для зарегистрированных пользователей
        await message.answer_photo(
            photo=photo_url,
            caption="Привет! 👋 Вы уже зарегистрированы в системе токенов. Добро пожаловать в нашего бота, созданного для обучения в онлайн! Здесь вы сможете:\n\n"
                    "1. Начать обучение 📚 — Откройте для себя увлекательные курсы и уроки по различным темам.\n"
                    "2. Задать вопрос ❓ — Если у вас возникли вопросы по курсам или обучению, не стесняйтесь спрашивать!\n"
                    "3. Посмотреть прогресс 📈 — Узнайте, как продвигается ваше обучение и какие задания еще предстоит выполнить.\n\n"
                    "Нажмите на кнопку ниже, чтобы начать свое обучение!"
    ...
    builder.add(types.KeyboardButton(text='Начать обучение 📚', web_app=webAppInfo))
